...
class ProseAnalysisConfig:
    ...
    target_grade_level_min: float = 12.0
    target_grade_level_max: float = 20.0
    long_sentence_threshold: int = 35
    citation_density_min_per_1000: float = 3.0
...
class BibliographyConfig:
    ...
    ``reserve_keys`` is the explicit allowlist of BibTeX keys permitted
    to live in ``references.bib`` without being cited from the
    manuscript. These are the "working reference library" entries the
    project intentionally keeps for cross-section reuse. When
    ``fail_on_unused`` is true, only the orphans that fall outside this
...
@dataclass
class ProjectConfig:
    """Top-level crescent_city configuration."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ProjectConfig":
        ...
        return cls(
            ...
            prose=ProseAnalysisConfig(
                ...
                citation_density_min_per_1000=float(prose_raw.get("citation_density_min_per_1000", 3.0)),
